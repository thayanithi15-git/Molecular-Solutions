...
export default function BiologyMain({handleSarVisionBiologics}) {
  ...
  const products = [
    {
      ...
      content: "Biologics SAR data analysis for Proteins, Peptides, Antibody and PROTACS",
    },
  ];
        ...
        {filteredProducts.map((item, index) => (
          <div className="products" key={index} onClick={() => handleItemClick(item, index)}>
            ...
            <div>
              ...
              <div className="content-nav" style={{marginLeft: "0%"}}>{item.content}</div>
            </div>
          </div>
        ))}
      </div>
